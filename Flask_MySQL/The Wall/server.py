...
def wall():
    if 'user' in session:
        query = "SELECT messages.message, messages.created_at, users.first_name, users.last_name, messages.id FROM messages JOIN users on (users.id = messages.user_id)"
        # comments.comment_text/JOIN comments on (comments.message_id = messages.id)
        messages = mysql.query_db(query)
        query2 = "SELECT comments.comment_text, comments.created_at, users.first_name, users.last_name, comments.message_id, messages.id FROM comments JOIN messages on (comments.message_id = messages.id) JOIN users on (comments.user_id = users.id)"
